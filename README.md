...
[![Tests: 17](https://img.shields.io/badge/tests-17-2A6DB2)](tests)
...
| Core logic  | Energy and range arithmetic is where a language model is least safe. | A deterministic route planner (`app/routing.py`) tracks state of charge and never plans to arrive below a 10% buffer. No LLM involved. |
...
Total: 599.2 km, 3 stops, ~54 min of charging, ~35.03 EUR. Paris to Dijon at 90% in a Tesla Model 3 (315.2 km) needs no stop.
...
```bash
pytest -m "not slow"   # 16 tests: core, API, tools; no network
pytest                 # all 17; the RAG test downloads Chroma's default embedding model once
```
